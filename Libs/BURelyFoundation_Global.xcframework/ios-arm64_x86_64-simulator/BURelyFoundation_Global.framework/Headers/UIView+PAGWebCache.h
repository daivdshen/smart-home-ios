...
#import "PAG_SDWebImageTransition.h"
#import "PAG_SDWebImageIndicator.h"

/**
 The value specify that the image progress unit count cannot be determined because the progressBlock is not been called.
 */
FOUNDATION_EXPORT const int64_t PAG_SDWebImageProgressUnitCountUnknown; /* 1LL */

typedef void(^PAG_SDSetImageBlock)(UIImage * _Nullable image, NSData * _Nullable imageData, PAG_SDImageCacheType cacheType, NSURL * _Nullable imageURL);

/**
 Integrates SDWebImage async downloading and caching of remote images with UIView subclass.
 */
@interface UIView (PAGWebCache)
 ...
 */
@property (nonatomic, strong, readonly, nullable) NSURL *sdPAG_imageURL;
 ...
 * The `totalUnitCount` and `completedUnitCount` will be reset to 0 after a new image loading start (change from current queue). And they will be set to `PAG_SDWebImageProgressUnitCountUnknown` if the progressBlock not been called but the image loading success to mark the progress finished (change from main queue).
 ...
 */
@property (nonatomic, strong, null_resettable) NSProgress *sdPAG_imageProgress;
 ...
 */
- (void)sdPAG_internalSetImageWithURL:(nullable NSURL *)url
                  placeholderImage:(nullable UIImage *)placeholder
                           options:(PAG_SDWebImageOptions)options
                           context:(nullable SDWebImageContext *)context
                     setImageBlock:(nullable PAG_SDSetImageBlock)setImageBlock
                          progress:(nullable SDImageLoaderProgressBlock)progressBlock
                         completed:(nullable PAG_SDInternalCompletionBlock)completedBlock;
 ...
 */
@property (nonatomic, strong, nullable) PAG_SDWebImageTransition *sdPAG_imageTransition;
 ...
 */
@property (nonatomic, strong, nullable) id<SDWebImageIndicator> sdPAG_imageIndicator;
